r"""Tiny JSON-backed config for the GitDeck GUI.

Stores just two things and never raises: the chosen theme ("light"/"dark") and a
short list of recently-opened repository folders.  On Windows the file lives at
``%LOCALAPPDATA%\GitDeck\config.json``; elsewhere it falls back to
``~/.gitdeck/config.json``.  Every function is defensive -- a corrupt or
...
def config_dir():
    ...
    ``%LOCALAPPDATA%\GitDeck`` on Windows, ``~/.gitdeck`` otherwise.
    """
...
def add_recent(path):
    """Push *path* to the front of the recent-repositories list (most recent first)."""
    if not path:
        return
    ap = _abs(path)
    cfg = load()
